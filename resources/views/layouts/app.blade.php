...
<body class="appMSPLayout {{ auth()->check() ? "appAuthorized" : "appAuthorizedNot" }} {{ (request()->root() === env('AM_URL')) ? "theme_am" : "theme_msp" }} @yield('body_class')">
<div id="app">
    <alerts></alerts>
    <!--
    ...
    <div class="app">
        ...
        <div class="appBrandMenu effectScale top left">
            ...
            @if (auth()->check() && auth()->user()->isRegistrationCompleted())
                <user-info :user-id="{{ auth()->id() }}" :data='@json(session('user'))'></user-info>
            @endif
